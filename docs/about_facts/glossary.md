...
This page contains a guide to terms and how they are used in the FACTS2 ecosystem.
...
This file is the human-editable artifact; once complete it drives the FACTS-experiment-builder (FEB) command `generate-compose`.

### experiment-compose.yaml
A Docker Compose file generated from a completed `experiment-config.yaml` by the `generate-compose` FEB CLI command. Contains one service definition per module (or per workflow for facts-total and ESL), with resolved image references, command arguments, volume mounts, and service dependencies. This file is used directly to run the experiment (e.g., `docker compose -f experiment-compose.yaml up`).
